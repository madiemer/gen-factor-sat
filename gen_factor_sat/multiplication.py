...
from gen_factor_sat.circuit import T
...
class KaratsubaMultiplication(Multiplication):
    ...
    def __init__(self, circuit, simple_mult, min_len=20):
    ...
    def multiply(self, factor_1: List[T], factor_2: List[T]) -> List[T]:
        ...
        if max_factor_length <= self.min_len:
            ...
            factor_1_sum = self.circuit.n_bit_adder(f1_high, f1_low, self.circuit.zero())
            factor_2_sum = self.circuit.n_bit_adder(f2_high, f2_low, self.circuit.zero())

            result_mid = self.multiply(factor_1_sum, factor_2_sum)
            result_mid = self.circuit.subtract(result_mid, result_high)
            result_mid = self.circuit.subtract(result_mid, result_low)

            # result = result_high * 2^(2 * half_factor_length) + result_mid * 2^(half_factor_length) + result_low
            shifted_high = self.circuit.shift(result_high, half_factor_length)
            result = self.circuit.n_bit_adder(shifted_high, result_mid, self.circuit.zero())

            shifted_high = self.circuit.shift(result, half_factor_length)
            result = self.circuit.n_bit_adder(shifted_high, result_low, self.circuit.zero())
...
class WallaceTreeMultiplier(Multiplication):
    ...
    def multiply(self, factor_1: List[T], factor_2: List[T]) -> List[T]:
        ...
        result = []
        last_carry = self.circuit.zero()
        for key in sorted(grouped_products):
            products = grouped_products[key]

            if len(products) == 1:
                x, = products
                sum, carry = self.circuit.half_adder(x, last_carry)
                last_carry = carry
                result.append(sum)
            else:
                x, y = products
                sum, carry = self.circuit.full_adder(x, y, last_carry)
                last_carry = carry
                result.append(sum)
    ...
    def _weighted_product(self, factor_1: List[T], factor_2: List[T]):
        for i, x in enumerate(factor_1):
            ...
            for j, y in enumerate(factor_2):
                ...
                weight_sum = w_x + w_y
                product = self.circuit.wire_and(x, y)
    ...
    def _add_layer(self, weight: int, products: List[T]) -> List[Tuple[int, T]]:
        if len(products) == 1:
            ...
            x, y = products
            sum, carry = self.circuit.half_adder(x, y)

            return [(weight, sum), (weight + 1, carry)]

        elif len(products) > 2:
            x, y, z = products[:3]
            sum, carry = self.circuit.full_adder(x, y, z)
